"""
Covertype Tabular Classification Experiment
=============================================

Tests whether CircleLayer and HelixLayer remain competitive with dense MLP
baselines on heterogeneous tabular classification.

Usage:
  cd src
  python covertype_run_experiment.py --quick                             # fast sanity check
  python covertype_run_experiment.py --quick --all-models                # quick all models
  python covertype_run_experiment.py --model-type helix_mlp --scale small
  python covertype_run_experiment.py --all-models --scale medium
  python covertype_run_experiment.py --all-models --sweep-scales
  python covertype_run_experiment.py --data-dir E:/ml_datasets --all-models --scale small
"""
...
from covertype_config import CovertypeConfig, apply_scale_preset, get_device, save_json
from covertype_data import make_covertype_dataloaders
from covertype_models import build_covertype_model, count_parameters
from covertype_train import evaluate, fit_covertype
from utils import set_seed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Covertype Tabular Classification Experiment")
...
def run_single(config: CovertypeConfig, cli_data_dir: str | None) -> dict:
    """Train one Covertype model variant."""
    print(f"\n{'='*60}")
    ...
    # Data
    dataloaders = make_covertype_dataloaders(config, cli_data_dir)
    ...
    model = build_covertype_model(config)
    n_params = count_parameters(model)
    print(f"Parameters: {n_params:,}")

    # Train
    result = fit_covertype(model, dataloaders, config)
    ...
    print(f"  Test weighted F1: {result['test_metrics']['weighted_f1']:.4f}")
...
def main() -> None:
    ...
    print("SUMMARY")
    print(f"{'='*60}")
    print(f"{'Model':<25} {'Scale':<8} {'Params':>10} {'Val F1':>10} {'Test Acc':>10} {'Test F1':>10} {'Time/Ep':>10}")
    print("-" * 85)
    for r in all_results:
        print(f"{r['model_type']:<25} {r['scale']:<8} {r['param_count']:>10,} "
              f"{r['best_val_macro_f1']:>10.4f} {r['test_accuracy']:>10.4f} "
              f"{r['test_macro_f1']:>10.4f} {r['mean_epoch_seconds']:>9.2f}s")
