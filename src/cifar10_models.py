"""Model definitions for Flattened CIFAR-10 HelixLayer experiment."""
...
class CircleLayer(nn.Module):
    """Learned circular feature layer.
...
class StandardMLP(nn.Module):
    def __init__(
        ...
        super().__init__()
        layers: list[nn.Module] = [nn.Flatten(), nn.Linear(input_dim, hidden_dim), nn.GELU()]
        if dropout > 0:
